...
extern double V2SquaredLength(Vector2 *v), V2Length();
double V2Dot(Point2* a,Point2* b);
double V2DistanceBetween2Points(Point2* a,Point2* b); 
extern Vector2 *V2Negate(Vector2 * v), *V2Normalize(Vector2 *v), *V2Scale(Vector2 *a,double newLen), *V2Add(Vector2 *a,Vector2 *b,Vector2 *c), *V2Sub();
extern Vector2 *V2Lerp(), *V2Combine(), *V2Mul(), *V2MakePerpendicular(Vector2 *a,Vector2 *ap);
extern Vector2 *V2New(), *V2Duplicate();
extern Point2 *V2MulPointByMatrix();
extern Matrix3 *V2MatMul();

extern Vector2 AngleBisectors(Point2 a,Point2 b,Point2 c);

extern double RegulaFalsi(), NewtonRaphson(), findroot();
extern void MidpointEllipse(float a,float b,std::vector<Point2>& pList);

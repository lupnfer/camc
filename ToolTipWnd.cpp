...
void CToolTipWnd::AddTool(CWnd *pWnd, CString strText, COLORREF clrTextColor/*=NULL*/, int timerDelay /*= -1*/, int timerDelayShow /*=-1*/)
{
   ...
   if ( m_toolPtr.Lookup( pWnd->GetSafeHwnd(), (void *&)m_stToolInfo ) )
   {
   ...
   m_stToolInfo = new BTOOLINFO;
   m_stToolInfo->strToolText = strText;
	m_stToolInfo->clrToolTextClr = clrTextColor;
   if ( timerDelay < 0 )
      m_stToolInfo->iTimerDelay = m_iDelay; // default value
   else
      m_stToolInfo->iTimerDelay = timerDelay;

   if (timerDelayShow < 0)
      m_stToolInfo->iTimerDelayShow = m_iDelayShow;
   else
      m_stToolInfo->iTimerDelayShow = timerDelayShow;


   m_toolPtr.SetAt( pWnd->GetSafeHwnd(), m_stToolInfo );
}
void CToolTipWnd::UpdateTipText(CWnd *pWnd,CString strTxt)
{
	 m_stToolInfo->strToolText = strTxt;
	m_toolPtr.SetAt( pWnd->GetSafeHwnd(), m_stToolInfo );
}

...
int CPropertiesViewBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_wndObjectCombo.AddString (_T("属性配置"));
...
void CPropertiesViewBar::InitPropList ()
{
	...
	CBCGPProp* pGroup1 = new CBCGPProp (_T("摄像头属性"));
	
	CBCGPProp* pProp=new CBCGPProp (_T("视频模式"), (_variant_t) _T("mode0"),
		_T("摄像头的增益控制"));
	pProp->AddOption(_T("mode1"));
	pProp->AddOption(_T("mode2"));
	pProp->AllowEdit(FALSE);
	pGroup1->AddSubItem(pProp);	

	pProp=new CBCGPProp (_T("采集数度"), (_variant_t) _T("High"),
		_T("摄像头的采集速度"));
	pProp->AddOption(_T("normal"));
	pProp->AllowEdit(FALSE);
	pGroup1->AddSubItem(pProp);	

	pProp=new CBCGPProp (_T("增益调节"), (_variant_t) 16,
		_T("摄像头的增益控制"));
	pProp->EnableSpinControl (TRUE, 0, 100);
	pGroup1->AddSubItem(pProp);	

	pProp=new CBCGPProp (_T("快门速度"), (_variant_t) 95,
		_T("摄像头的快门速度，单位毫秒ms"));

...
void CBlobContour::FindDominantPoints(std::vector<CvPoint>& dpList,double p1,double p2,double p3,double p4)
{
	if ( m_contour == NULL || m_contour->total <= 300 )
	{
		return;
	}
	CvSeq* dps = NULL;
	dps=cvFindDominantPoints(GetContourPoints(),m_parentStorage,CV_DOMINANT_IPAN,p1,p2,p3,p4);
	
	for (int i = 0; i < dps->total; i++)
	{
		int idx = *(int *) cvGetSeqElem(dps, i);
		CvPoint pt = *(CvPoint *) cvGetSeqElem(m_contourPoints, idx);
		dpList.push_back(pt);
	}
}
...
CvBox2D CBlobContour::GetMinBoundingRect(){
	CvBox2D box2d;
	if(!m_contourPoints){
		GetContourPoints();		
	}
	box2d=cvMinAreaRect2(m_contourPoints,m_parentStorage);
	return box2d;
}
...
namespace svg
{
    ...
    unsigned parser::parse_matrix(const char* str)
    {
        ...
        m_path.transform() *= trans_affine(args[0], args[1], args[2], args[3], args[4], args[5]);
        return len;
    }
    ...
    unsigned parser::parse_translate(const char* str)
    {
        ...
        if(na == 1) args[1] = 0.0;
        m_path.transform() *= trans_affine_translation(args[0], args[1]);
        return len;
    }
    ...
    unsigned parser::parse_rotate(const char* str)
    {
        ...
        if(na == 1) 
        {
            m_path.transform() *= trans_affine_rotation(deg2rad(args[0]));
        }
        else if(na == 3)
        {
            m_path.transform() *= trans_affine_translation(-args[1], -args[2]);
            m_path.transform() *= trans_affine_rotation(deg2rad(args[0]));
            m_path.transform() *= trans_affine_translation(args[1], args[2]);
        }
        else
    ...
    unsigned parser::parse_scale(const char* str)
    {
        ...
        m_path.transform() *= trans_affine_scaling(args[0], args[1]);
        return len;
    }
    ...
    unsigned parser::parse_skew_x(const char* str)
    {
        ...
        m_path.transform() *= trans_affine_skewing(arg, 0.0);
        return len;
    }
    ...
    unsigned parser::parse_skew_y(const char* str)
    {
        ...
        unsigned na = 0;
        unsigned len = parse_transform_args(str, &arg, 1, &na);
        m_path.transform() *= trans_affine_skewing(0.0, arg);
        return len;
    }

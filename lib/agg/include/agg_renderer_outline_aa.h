...
namespace agg
{
    ...
    public:
        ...
        bool step_hor()
        {
            ...
            cover_type* p0 = m_covers + max_half_width + 2;
            cover_type* p1 = p0;

            *p1++ = (cover_type)m_ren.cover(s1);

            dy = 1;
            while((dist = m_dist[dy] - s1) <= m_width)
            {
                *p1++ = (cover_type)m_ren.cover(dist);
                ++dy;
            }
            ...
            while((dist = m_dist[dy] + s1) <= m_width)
            {
                *--p0 = (cover_type)m_ren.cover(dist);
                ++dy;
            }
    ...
    public:
        ...
        { 
            ...
            for(i = 0; i < aa_num; i++)
            {
                m_gamma[i] = value_type(gamma_function(double(i) / aa_mask) * aa_mask);
            }
        }

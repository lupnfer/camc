...
namespace agg
{
    ...
    //------------------------------------------------------------------------
    template<class T> void scanline_u<T>::add_span(int x, unsigned num, unsigned cover)
    {
        x -= m_min_x;
        memset(m_covers + x, cover, num);
        if(x == m_last_x+1)
        {
            m_cur_span->len = (int16)(m_cur_span->len + num);
        }
        else
        {
            m_cur_span++;
            m_cur_span->x      = (int16)(x + m_min_x);
            m_cur_span->len    = (int16)num;
            m_cur_span->covers = m_covers + x;
        }
        m_last_x = x + num - 1;
    }

...
namespace SXF{
	...
	IplImage* SaveBmp1()
	{
		...
		int i, j, nIndex;
		int nWidth1 = ((nWidth & 3) > 0)*4 + nWidth;
		for(i = 0, nIndex = -1; i < nHeight; i++)
		{
			for(j = 0; j < nWidth1; j++)
			{
	...
	int ConnectionFilter()
	{
		...
		if(pbIsVisited && pBlock && pPixelPosition)	// 连通过滤
		{
			while(1)
			{
				nBlockCount = SXF::FindConnectedRegion(pbIsForeground, pBlock, pPixelPosition, pbIsVisited, 0, nWidth, 0, nHeight);
				bool bNoNewSmallArea = 1;
				for(i = 0; i < nBlockCount; i++)
					if(pBlock[i].area < 100)	// 删除面积过小的区域
					{
	...
	int Process()
	{
		...
		for(i = 0, nIndex = 0; i < nHeight; i++)
			for(j = 0; j < nWidth; j++, nIndex++)
			{
				...
				if(pR[nIndex] < 140 && pB[nIndex] < 150 && pG[nIndex] > 80)
				{
					dRatio = pG[nIndex] / (double)pB[nIndex] + pG[nIndex] / (double)pR[nIndex];
				}
				pbIsForeground[nIndex] = (dRatio <= 2.9);// || dRatio >= 4.5);

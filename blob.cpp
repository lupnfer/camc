...
CBlob& CBlob::operator=(const CBlob &src )
{
	if( this != &src )
	{
		...
		m_minBoundBox = src.m_minBoundBox;
	...
	//src.m_dpList.clear();
	/*for(std::vector<CvPoint>::iterator it=m_dpList.begin();it!=m_dpList.end;++it){
		
	}*/
	std::copy(src.m_dpList.begin(),src.m_dpList.end(),m_dpList.begin());
	return *this;
}
...
CvBox2D CBlob::GetMinBoundingRect(){
	if(m_minBoundBox.size.width<0)
		m_minBoundBox=m_externalContour.GetMinBoundingRect();
	return m_minBoundBox;
}


void CBlob::FillBlob( IplImage *imatge, CvScalar color, int offsetX /*=0*/, int offsetY /*=0*/) 					  
{
	cvDrawContours( imatge, m_externalContour.GetContourPoints(), color, color,0, CV_FILLED, 8,cvPoint(offsetX,offsetY));
	t_contourList::iterator itContour; 
	itContour = m_internalContours.begin();

	while (itContour != m_internalContours.end() )
	{
		cvDrawContours( imatge,itContour->GetContourPoints(), CV_RGB(0x00,0x00,0x00), CV_RGB(0x00,0x00,0x00),0, CV_FILLED, 8,cvPoint(offsetX,offsetY));
		itContour++;
	}
}

void CBlob::DrawBlobContour(IplImage* image,CvScalar color,int offsetX, int offsetY)
{
	cvDrawContours( image, m_externalContour.GetContourPoints(), color, color,0, 1, 8,cvPoint(offsetX,offsetY));
	t_contourList::iterator itContour; 
	itContour = m_internalContours.begin();

	while (itContour != m_internalContours.end() )
	{
		cvDrawContours( image,itContour->GetContourPoints(), CV_RGB(0x00,0x00,0x00), CV_RGB(0x00,0x00,0x00),0, 1, 8,cvPoint(offsetX,offsetY));
		itContour++;
	}
}
std::vector<CvPoint>& CBlob::FindDominantPoints(double p1,double p2,double p3,double p4)
{
	if(m_dpList.empty()){
		m_externalContour.FindDominantPoints(m_dpList,p1,p2,p3,p4);
		t_contourList::iterator it=m_internalContours.begin();
		for(;it!=m_internalContours.end();++it){
			it->FindDominantPoints(m_dpList,p1,p2,p3,p4);
		}
	}
	return m_dpList;
}
void CBlob::DrawDominantPoints(IplImage* img,CvScalar color)
{
	std::vector<CvPoint>::iterator it=m_dpList.begin();
	for(;it!=m_dpList.end();++it){
		cvDrawCircle(img,*it,4,color,1);
	}
}
